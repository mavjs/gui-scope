...
if [[ "$(uname -s)" == "Darwin" ]]; then
  ...
  echo "⚠️  Linux (Wayland and X11 both supported — auto-detected from \$XDG_SESSION_TYPE):"
  echo "   - Wayland: requires an active, logged-in graphical session with"
  echo "     xdg-desktop-portal + xdg-desktop-portal-gnome running. The first"
  echo "     click/key action will show a GNOME 'Allow input control?' dialog —"
  echo "     approve it once. The session is then cached at"
  echo "     ~/.config/gui-scope/wayland_token."
  echo "   - X11: no consent dialog needed; requires an EWMH-compliant window"
  echo "     manager so gui-scope can raise/focus the target app's window."
  echo "   - For Java/Swing apps (e.g. Burp Suite): install java-atk-wrapper"
  echo "     and ensure AT-SPI accessibility is enabled."
  echo "   See HOWTO.md's 'Linux setup' section for details."
fi
